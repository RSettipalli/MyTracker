	...
	public String material(@ModelAttribute("user") User userInfo, Model model) {
		...
		if (userInfo != null) {
			List<IMY_MAT_ONLINE> imyMatOnline = materialManager
					.getMaterialInfo();
			// List<IMY_MAT_WERKS> imyMatPlant =
			// materialManager.getMaterialPlantDetails();
			// List<IMY_MAT_STORAGE_DETIALS> imyMatStorageDetailsList =
			// materialManager.getMaterialStorageDetails();
...
//		MaterialForm materialForm = new MaterialForm();
	...
	public String invoice(@ModelAttribute("user") User userInfo, Model model) {
		logger.debug("BEGIN");
		if (userInfo != null) {
			List<IMY_MGOL_INV_DETAIL> invDetail = inventoryManager
					.getInventoryDetail(userInfo.getCompanyId());
			model.addAttribute("invDetail", invDetail);

			IMY_MGOL_INV_HEADER invoiceHeader = inventoryManager
					.getInventoryHeader(userInfo.getCompanyId());
			model.addAttribute("invoiceHeader", invoiceHeader);

			List<IMY_MGOL_INV_HEADER_COMMEN> invHeaderComments = inventoryManager
					.getInvHeaderCommentDetails(userInfo.getCompanyId());
			model.addAttribute("invoiceOrderHeaderComments", invHeaderComments);

			List<IMY_MGOL_SO_DETAIL_COMMENT> invDetailComments = inventoryManager
					.getInvDetailComment(userInfo.getCompanyId());
			model.addAttribute("invDetailComments", invDetailComments);

			List<IMY_MGOL_INV_ITEM_ATMT> invItemAttachments = inventoryManager
					.getInvAttachement(userInfo.getCompanyId());
	...
	public String orderInfo(@ModelAttribute("user") User userInfo, Model model) {
		logger.debug("BEGIN");
		if (userInfo != null) {
			List<IMY_MGOL_OD_DETAIL> orderDetail = orderManager
					.getOrderDetail(userInfo.getCompanyId());
			model.addAttribute("orderDetail", orderDetail);

			IMY_MGOL_OD_HEADER orderHeader = orderManager
					.getOrderHeaderDetail(userInfo.getCompanyId());
			model.addAttribute("orderHeader", orderHeader);

			List<IMY_MGOL_OD_HEADER_COMMENT> odHeaderComments = orderManager
					.getOrderHeaderCommentDetails(userInfo.getCompanyId());
			model.addAttribute("orderHeaderComments", odHeaderComments);

			List<IMY_MGOL_SO_DETAIL_COMMENT> odDetailComments = orderManager
					.getOrderDetailComment(userInfo.getCompanyId());
			model.addAttribute("odDetailComments", odDetailComments);

			List<IMY_MGOL_OD_ITEM_ATTACHM> odDetailItemAttachments = orderManager
					.getOrderDetailAttachement(userInfo.getCompanyId());
			model.addAttribute("odDetailItemAttachments",
					odDetailItemAttachments);
		}
		logger.debug("END");
		return "OrderInfo";
	}
